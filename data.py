...
def get_adg(args):
    ...
    data = pd.read_csv(args.data_dic+'/train.csv',names = ["text","label"] ,header=0)
    test_data = pd.read_csv(args.data_dic+'/test.csv',names = ["text","label"], header=0)
    test_data['train_id'] = test_data["text"]
    ...
    '''def modifylabel(row):
        ...
        if row!="label":
            #print("row", row)
            return int(row) - 1'''
    #print("test label:", test_data["label"])
    #test_data["label"] = test_data["label"]#.apply(modifylabel)

    #print("test_data['train_id']",test_data['train_id'])
    ...
    data2 = data#.sample(frac= 0.8, random_state=2333)
        #0。3
    ...
    if args.num_labeled == 10000:
        #0.4
        data2 = data.sample(frac= 0.5, random_state=2333)'''
    data2['train_id'] = data2["text"]
...
def get_dis(args):
    ...
    data_la = pd.read_csv(args.data_dic+'/la.csv',names = ["text","label"] ,header=0)
    data_un = pd.read_csv(args.data_dic + '/un.csv', names=["text", "label"], header=0)
    test_data = pd.read_csv(args.data_dic+'/test.csv',names = ["text","label"], header=0)
    test_data['train_id'] = test_data["text"]
    test_data['train_id'] = test_data['train_id'].apply(tokenizen)
    ...
    #if args.num_labeled == 100:
    data_la = data_la.sample(frac= 0.8, random_state=2333)
        #0。3
    '''if args.num_labeled == 1000:
        # data2 = data.sample(frac= 0.3, random_state=2333)
        data2 = data.sample(frac= 0.4, random_state=2333)
    if args.num_labeled == 10000:
        #0.4
        data2 = data.sample(frac= 0.5, random_state=2333)'''
    data_la['train_id'] = data_la["text"]
    data_la['train_id'] = data_la['train_id'].apply(tokenizen)
    #data2["label"] = data2["label"]#.apply(modifylabel)

    data_un2 = data_un.sample(frac=0.6, random_state=2333)
    data_un2['train_id'] = data_un2["text"]
    data_un2['train_id'] = data_un2['train_id'].apply(tokenizen)

    train_data_la, dev_data_la, train_label_la, dev_label_la = train_test_split(data_la['train_id'].values,
                                                                              data_la['label'].values, test_size=int(0.2 * args.num_labeled),
                                                                              train_size=int(0.8 *args.num_labeled),
                                                                              random_state=2333)

    train_data_un, dev_data_un, train_label_un, dev_label_un = train_test_split(data_un2['train_id'].values,
                                                                                data_un2['label'].values,
                                                                                test_size=int(0.2 * args.num_unlabeled),
                                                                                train_size=int(0.8*args.num_unlabeled),
                                                                                random_state=2333)

    train_data = np.concatenate((train_data_la, train_data_un))
    dev_data = np.concatenate((dev_data_la, dev_data_un))
    train_label = np.concatenate((train_label_la, train_label_un))
    dev_label = np.concatenate((dev_label_la, dev_label_un))
